...
\definecolor{darkblue}{rgb}{.10,.10,.40}
...
\begin{itemize}
	...
	\item This can be used for machine translation, summarization, chatbots/dialog systems, and \textbf{sequences-to-sequence} tasks
\begin{center}
...
\begin{itemize}
	\item {\small Attention-based decoding adds another\textsuperscript{\teeny{*}} FF network ($\mathbf{a}$) that takes as input the encoder's hidden state ($\mathbf{h}$) and the decoder's hidden state ($\mathbf{z}$), and outputs a probability for each source word at each time step (when and where to pay attention) :}\\
		% $e_{i,j} = a(z_{i-1}, h_j) = v^{\top}_a \tanh (W_a s_{i-1} + U_a h_j) $

\begin{center}
\includegraphics[height=0.40\textheight]{images/cho_nvidia_fig-3_attention.png}
\end{center}

	\item Because attention outputs probabilities, it requires expensive normalization (via softmax), at each decoding timestep
	\pause
	\item The attention weights can also function as soft word alignments. They're trained on target-side MLE
\end{itemize}
\teeny{Attention isn't actually another network, but rather another component in the network.  Image courtesy of \url{http://devblogs.nvidia.com/parallelforall/introduction-neural-machine-translation-gpus-part-3}}
\end{frame}
...
\begin{itemize}
	...
	\item \url{https://arxiv.org/abs/1703.01619}
\end{itemize}
\end{tiny}
%\end{minipage}
\end{block}
...
\begin{scriptsize}
\url{https://github.com/jonsafari/nmt-list}
\end{scriptsize}
\end{block}
