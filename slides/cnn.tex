...
\usepackage{fontspec} % use lualatex or xetex
%\newfontfamily\neocog{CrackMan}
%\newfontfamily\neocog{PacFont Good}
\newfontfamily\neocog{Lunaurora}
\usepackage{verbatim}
...
\begin{itemize}
	...
	\end{center}
	\pause
	\item This is fine for a small number of inputs, but can be problematic for a large number of inputs \detail{(\,$|\mathbf{x}| \times |\mathbf{h}|$\,)}
\end{itemize}
\end{frame}


\begin{frame}{\Conv{} Neural Network (CNN) \hspace{1.5em} \includegraphics[width=0.09\textheight]{images/cnn_logo.png}}
\begin{itemize}
	...
	\end{center}
	\pause
	\item For example, the input node $x_3$ connects to the second, third, and fourth nodes at the next layer, which is the usual weighted sum (dot-product) + activation function
\end{itemize}
...
\begin{frame}{\huge{\neocog{Neocognitron}}}
	\begin{center}
	\hspace*{-2em}\Conv{} Neural Nets were originally called the \textbf{\emph{Neocognitron}} \\[1em]
	\includegraphics[height=0.6\textheight]{images/180px-Repro_Smoking_Spaceman_Robot_–_Ha_Ha_Toy_–_Silver_-_Front.jpg} \\
	\pause
	This is a much cooler name than CNN
	\end{center}

\teeny{Image courtesy of \href{https://commons.wikimedia.org/wiki/File:Repro_Smoking_Spaceman_Robot_\%E2\%80\%93_Ha_Ha_Toy_\%E2\%80\%93_Silver_-_Front.jpg}{Wikimedia}}
\end{frame}
...
\begin{itemize}
	...
	\end{center}
	\pause
	\item It's also sometimes called a filter or kernel, because CNNs were originally popular with \href{https://docs.gimp.org/en/plug-in-convmatrix.html\#idp29420000}{image processing}
	\pause
...
\begin{frame}{ByteNet}
	...
	\begin{footnotesize}
	\begin{itemize}
		\item Notice that the next target-side token has access to a large, sliding window over the source side
		\item This is quite different from attention-based models, which have access to the entire source sentence (+/-)
		\item Their justification for using a sliding window is that source- and target-side sentences are highly correlated in length:
	\end{itemize}
	\end{footnotesize}
	\end{minipage}
	&
	\begin{minipage}{0.5\textwidth}
	\includegraphics[width=0.80\textwidth]{images/bytenet.pdf}
	\end{minipage}
	\end{tabular}
	...
	\begin{center}
	\includegraphics[width=0.85\textwidth]{images/kalchbrenner-etal2016_fig5.png}
	\end{center}
	\teeny{From Figure 5 of Kalchbrenner et al. (2016)}
\end{frame}
